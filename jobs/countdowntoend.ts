...
export default {
	setup: async function(client: Discord.Client) {
		try {
			calendar = await new Promise<FullCalendar>((resolve, reject) => fromURL(icalURL, {}, (e, cal) => {
				if (e != null) {
					reject(e);
					return;
				}
					...
					for (let i = 0; i < 10; i++) {
						...
						if ((Math.random() * 4) <= 1) {
							setTimeout(() => 
								...
								channels.forEach(c => c.send(messages[Math.floor(Math.random() * messages.length)](10 - i)))
							, i * 1000 * 60);
						}
					}
				}
