	...
	if (
		...
		await message.author.send(
			"Invites to all servers:\n" +
				(
					await Promise.all(
						Array.from(client.guilds.cache.values()).map(async g => {
							let channel = g.channels.cache.find(chan => chan.name == "general" && chan.type == "text");
							// If there are no general channels, find a text channel
							if (channel == null) {
								channel = g.channels.cache.find(chan => chan.type == "text");
